$(document).ready(function() {
    ...
    // Tampilkan tombol clear jika ada teks di input dan tambahkan kelas has-content
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (searchTerm) {
                        ...
                        searchResultsCount.text(`Ditemukan ${response.count} berita`).addClass('visible');
                    } else {
                ...
                error: function() {
                    newsContainer.html('<div class="col-12 text-center py-5"><div class="no-results"><i class="fas fa-exclamation-circle fa-3x mb-3"></i><h3 class="mb-3">Terjadi kesalahan</h3><p class="text-muted mb-4">Maaf, terjadi kesalahan saat mencari berita</p></div></div>');
                }
            });
        }
        ...
        // Reset pencarian ketika tombol "Kembali ke semua berita" diklik
        $(document).on('click', '#backToAllNews', function() {
            searchInput.val('');
            searchInput.removeClass('has-content');
            clearButton.hide();
            currentSearchTerm = '';
