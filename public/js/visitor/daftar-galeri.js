$(document).ready(function() {
    ...
    // Tampilkan tombol clear jika ada teks di input dan tambahkan kelas has-content
        ...
        function performSearch() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    galeriContainer.html(response.html);

                    // Update URL tanpa refresh halaman
                    const url = new URL(window.location);
                    if (searchTerm) {
                        url.searchParams.set('search', searchTerm);
                        searchResultsCount.text(`Ditemukan ${response.count} galeri`).addClass('visible');
                    } else {
                ...
                error: function() {
                    galeriContainer.html('<div class="col-12 text-center py-5"><div class="no-results"><i class="fas fa-exclamation-circle fa-3x mb-3"></i><h3 class="mb-3">Terjadi kesalahan</h3><p class="text-muted mb-4">Maaf, terjadi kesalahan saat mencari galeri</p></div></div>');
                }
            });
        }
        ...
        // Reset pencarian ketika tombol "Kembali ke semua galeri" diklik
        $(document).on('click', '#backToAllGalleries', function() {
            searchInput.val('');
            searchInput.removeClass('has-content');
            clearButton.hide();
            currentSearchTerm = '';
